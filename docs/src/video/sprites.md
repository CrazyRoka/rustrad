...
$$\text{Screen Byte} \leftarrow \text{Screen Byte} \oplus \text{Sprite Byte}$$
...
$$\text{Screen Byte} \leftarrow (\text{Screen Byte} \text{ AND } \text{Mask}) \text{ OR } \text{Sprite Data}$$
...
* **Magnification:** Independent horizontal and vertical scaling factors ($1\times, 2\times, 4\times$) can be applied to match the main screen's mode resolution (e.g., $4\times$ magnification for Mode 0).
