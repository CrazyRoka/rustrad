...
#### Analog Stereo Output Panning and Mixing
Unlike standard mono systems, the physical mainboard circuitry of the Amstrad CPC maps the three independent analog sound channels from the AY-3-8912 to the stereo jack socket (3.5mm headphone jack) on the side of the chassis as follows:
* **Analog Channel A**: Routed hard-left via coupling capacitor `C117`.
* **Analog Channel C**: Routed hard-right via coupling capacitor `C120`.
* **Analog Channel B**: Centered. It is mixed equally to both Left and Right channels via resistor networks `R118`/`R119`.

*Motherboard Mono Summing:* On-board mono audio (for the internal CPC speaker and Pin 1 of the Expansion Connector) is generated by summing Channels A, B, and C together through mixing resistors `R114`, `R115`, and `R116` to drive the on-board LM386 audio amplifier.